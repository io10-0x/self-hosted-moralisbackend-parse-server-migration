...
Parse.Cloud.define('watchContractEvent', async ({ params }) => {
  let provider;
  if (params['chainId'] == 1337) {
    provider = new ethers_1.ethers.WebSocketProvider('http://127.0.0.1:8545/');
  } else {
    provider = new ethers_1.ethers.WebSocketProvider(process.env.SEPOLIA_RPC_URL);
  }
...
Parse.Cloud.afterSave('ItemListed', async function (request) {
...
Parse.Cloud.afterSave('ItemUnlisted', async function (request) {
  let logger = request.log;
...
Parse.Cloud.afterSave('ItemBought', async function (request) {
  let logger = request.log;
...
Parse.Cloud.afterSave('ItemUpdated', async function (request) {
  let logger = request.log;
